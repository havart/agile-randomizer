...
import {
  compressSettings,
  decompressSettings
} from "../../helpers/data-compression.helper";

const commonRoles: string[] = ["RETRO STAR"];
...
export class DataStorageService {
  ...
  private _slackTeamChannel: string;
  private _isPrivateChannel = false;
  private _isPrivateChannelCommon = false;
  private _state = new ReplaySubject<DataStorageInterface>(1);

  public get isDevRoles(): boolean {
    return this._isPrivateChannel;
  }

  public get isPrivateChannelCommon(): boolean {
    return this._isPrivateChannelCommon;
  }

  public set isPrivateChannelCommon(isPrivate: boolean) {
    this._isPrivateChannelCommon = isPrivate;
  }
  ...
  public get slackTeamChannel(): string {
    return this._slackTeamChannel || defaultslackTeamChannel;
  }

  public set slackTeamChannel(slackTeamChannel: string) {
    this._slackTeamChannel = slackTeamChannel;
  }

  public getState$(): Observable<string> {
    return this._state.pipe(
      map((state: DataStorageInterface) =>
        JSON.stringify(compressSettings(state))
      )
    );
  }

  public restoreState(
    minimizedServiceData: MinimizedDataStorageInterface
  ): void {
