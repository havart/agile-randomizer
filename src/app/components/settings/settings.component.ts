...
export class SettingsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.dataStorageService
      ...
      .pipe(
        ...
        map(
          (state: string) =>
            `${window.location.href.replace("settings", "")}?data=${state}`
        ),
  ...
  public removeRole(roleIndex: number): void {
    this.roles = [
      ...this.roles.slice(0, roleIndex),
      ...this.roles.slice(roleIndex + 1, this.roles.length)
    ];
  }
  ...
  public onTeamMemberValueChange(event: Event, index: number): void {
    ...
    this.teamMembers = [
      ...
      newValue,
      ...this.teamMembers.slice(index + 1, this.teamMembers.length)
    ];
  }
  ...
  public copyLinkToClipboard(): void {
    this.snackBar.open("Ссылка скопирована в буфер обмена", "", {
      duration: 1000
    });
  }
